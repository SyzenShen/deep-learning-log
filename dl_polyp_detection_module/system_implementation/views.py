...
class MLTaskViewSet(viewsets.ModelViewSet):
  ...
  def trigger(self, request):
    ...
    if task_type == 'polyp_detect':
        try:
            ...
            # Assuming file path is accessible via file object
            # file_upload.File model has 'file' field which is a FileField
            ...
            if file_obj.file:
                 ...
                 task.status = 'failed'
                 task.result = {'error': 'No file associated'}
                 task.save()
  ...
  def explain(self, request, pk=None):
      ...
      file_name = task.file.original_filename if task.file else "unknown_image"
